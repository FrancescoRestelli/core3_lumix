...
#define CTS_ASSERTED     (_inverse ? (digitalRead_fast(_ctsPin) == HIGH) : (digitalRead_fast(_ctsPin) == LOW))
...
uint8_t rx_divider  = 1;
uint16_t rx_compare = 0;
tc_clock_prescaler rx_prescaler = TC_CLOCK_PRESCALER_DIV1;

uint8_t tx_divider  = 1;
uint16_t tx_compare = 0;
tc_clock_prescaler tx_prescaler = TC_CLOCK_PRESCALER_DIV1;  


  

Adafruit_ZeroTimer _rx_timer = Adafruit_ZeroTimer(3);
Adafruit_ZeroTimer _tx_timer = Adafruit_ZeroTimer(4);
...
void SlowSoftSerial::begin(double baudrate, uint16_t config) {
    ...
    if (baudrate < _SSS_MIN_BAUDRATE) {
	 ...
	 baudrate;	
	//RX
	float freq =  baudrate*4; 	
	

  if ((freq < 24000000) && (freq > 800)) {
    rx_divider = 1;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV1;
    rx_compare = 48000000/freq;
  } else if (freq > 400) {
    rx_divider = 2;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV2;
    rx_compare = (48000000/2)/freq;
  } else if (freq > 200) {
    rx_divider = 4;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV4;
    rx_compare = (48000000/4)/freq;
  } else if (freq > 100) {
    rx_divider = 8;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV8;
    rx_compare = (48000000/8)/freq;
  } else if (freq > 50) {
    rx_divider = 16;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV16;
    rx_compare = (48000000/16)/freq;
  } else if (freq > 12) {
    rx_divider = 64;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV64;
    rx_compare = (48000000/64)/freq;
  } else if (freq > 3) {
    rx_divider = (uint8_t)256;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV256;
    rx_compare = (48000000/256)/freq;
  } else if (freq >= 0.75) {
    rx_divider = (uint8_t)1024;
    rx_prescaler = TC_CLOCK_PRESCALER_DIV1024;
    rx_compare = (48000000/1024)/freq;
  } else {
    Serial.println("Invalid RX frequency");
    while (1) delay(10);
  }
#ifdef SERIALDEBUG
  Serial.print("rx Divider:"); Serial.println(rx_divider);
  Serial.print("rx Compare:"); Serial.println(rx_compare);
  Serial.print("rx Final freq:"); Serial.println((int)(48000000/rx_compare));
#endif	
	//TX
	 freq = baudrate;
  ...
  if ((freq < 24000000) && (freq > 800)) {
    tx_divider = 1;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV1;
    tx_compare = 48000000/freq;
  } else if (freq > 400) {
    tx_divider = 2;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV2;
    tx_compare = (48000000/2)/freq;
  } else if (freq > 200) {
    tx_divider = 4;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV4;
    tx_compare = (48000000/4)/freq;
  } else if (freq > 100) {
    tx_divider = 8;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV8;
    tx_compare = (48000000/8)/freq;
  } else if (freq > 50) {
    tx_divider = 16;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV16;
    tx_compare = (48000000/16)/freq;
  } else if (freq > 12) {
    tx_divider = 64;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV64;
    tx_compare = (48000000/64)/freq;
  } else if (freq > 3) {
    tx_divider = (uint8_t)256;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV256;
    tx_compare = (48000000/256)/freq;
  } else if (freq >= 0.75) {
    tx_divider = (uint8_t)1024;
    tx_prescaler = TC_CLOCK_PRESCALER_DIV1024;
    tx_compare = (48000000/1024)/freq;
  } else {
    Serial.println("Invalid TX frequency");
    while (1) delay(10);
  }
#ifdef SERIALDEBUG

  Serial.print("tx Divider:"); Serial.println(tx_divider);
  Serial.print("tx Compare:"); Serial.println(tx_compare);
  Serial.print("tx Final freq:"); Serial.println((int)(48000000/tx_compare));
#endif	

  //rx
  _rx_timer.enable(false);
  _rx_timer.configure(rx_prescaler,       // prescaler
          TC_COUNTER_SIZE_16BIT,       // bit width of timer/counter
          TC_WAVE_GENERATION_MATCH_PWM // frequency or PWM mode
          );

  _rx_timer.setCompare(0, rx_compare);
  _rx_timer.setCallback(true, TC_CALLBACK_CC_CHANNEL0, _rx_timer_trampoline);
	//---TX

  _tx_timer.enable(false);
  _tx_timer.configure(tx_prescaler,       // prescaler
          TC_COUNTER_SIZE_16BIT,       // bit width of timer/counter
          TC_WAVE_GENERATION_MATCH_PWM // frequency or PWM mode
          );

  _tx_timer.setCompare(0, tx_compare);
  _tx_timer.setCallback(true, TC_CALLBACK_CC_CHANNEL0, _tx_trampoline);
    ...
    digitalWrite_fast(_txPin, _SSS_STOP_LEVEL);
...
void SlowSoftSerial::end(bool releasePins) {
	...
	_tx_timer.enable(false);    // called first to avoid any conflict for variables
    _rx_timer.enable(false);
...
int SlowSoftSerial::getTXBufferSize(void) {
...
size_t SlowSoftSerial::write(uint8_t chr) {
    ...
    if (!_tx_running) {		
#ifdef ONEWIRE  
	//Serial.println("TX not running disabled interrupt");
	detachInterrupt(digitalPinToInterrupt(_rxPin));
#endif	
	
	_tx_running = true;
    _tx_timer.setCallback(true, TC_CALLBACK_CC_CHANNEL0, _tx_trampoline);
	_tx_timer.enable(true);
...
void SlowSoftSerial::_tx_handler(void) {
    ...
    if (_tx_bit_count > 0) {
        ...
        digitalWrite_fast(_txPin, _tx_data_word & 0x01);
        _tx_data_word >>= 1;
        _tx_bit_count--;
        return;
    }
    ...
    if (_tx_buffer_count == 0 ) {
		...
		_tx_running = false;	        
            ...
            _rx_timer.enable(false);
			attachInterrupt(digitalPinToInterrupt(_rxPin), _rx_start_trampoline, _inverse ? RISING : FALLING);
			//Serial.println(" oneWire enable2 rx pin");		
#endif			
    ...
    _tx_buffer_count--;
    digitalWrite_fast(_txPin, _SSS_START_LEVEL);
...
void SlowSoftSerial::_rx_start_handler(void) {	
			_rx_timer.setCallback(true, TC_CALLBACK_CC_CHANNEL0, _rx_timer_trampoline);
		_rx_timer.enable(true);
		#ifdef SERIALDEBUG_VERBOSE
...
void SlowSoftSerial::_rx_timer_handler(void) {	
    ...
    // and the difference is not likely to be huge.
        ...
        case _SSS_OP_START:
#ifdef SERIALDEBUG_VERBOSE
				Serial.println("_SSS_OP_START");
#endif	
            // We are somewhere in the middle of the start bit.
            // Just make sure it's still a valid start bit.
            if (digitalRead_fast(_rxPin) != (_inverse ? HIGH : LOW)) {
				#ifdef SERIALDEBUG_VERBOSE
					Serial.println("_SSS_OP_START noise");
				#endif
        ...
        case _SSS_OP_VOTE0:
#ifdef SERIALDEBUG_VERBOSE
			Serial.println("_SSS_OP_VOTE0");
#endif		
            // We're ready to take the first sample of a data or parity bit.
            _rx_bit_value = digitalRead_fast(_rxPin);
            break;
            
        case _SSS_OP_VOTE1:
#ifdef SERIALDEBUG_VERBOSE
					Serial.println("_SSS_OP_VOTE1");
#endif
            // We're still in the middle of a data or parity bit.
            // Just make sure it hasn't changed on us.
            if (digitalRead_fast(_rxPin) != _rx_bit_value) {
                // stop the timer and go back to waiting for a start bit.
                // must have been noise, or baud rate error, or something.
#ifdef SERIALDEBUG							
				Serial.println("_SSS_OP_VOTE1 noise");
#endif				
                _rx_timer.enable(false);
                attachInterrupt(digitalPinToInterrupt(_rxPin), _rx_start_trampoline, _inverse ? RISING : FALLING);
            }
...
#ifdef SERIALDEBUG_VERBOSE
			Serial.println("_SSS_OP_STOP");
#endif
            if (digitalRead_fast(_rxPin) != (_inverse ? LOW : HIGH)) {
                // stop the timer and go back to waiting for a start bit.
                // must have been noise, or baud rate error, or something.
#ifdef SERIALDEBUG			
				Serial.println("_SSS_OP_STOP noise");
#endif				
                _rx_timer.enable(false);
                attachInterrupt(digitalPinToInterrupt(_rxPin), _rx_start_trampoline, _inverse ? RISING : FALLING);
            }
        ...
        case _SSS_OP_FINAL:
            ...
            if (digitalRead_fast(_rxPin) == (_inverse ? LOW : HIGH)) {
                ...
                if (_rx_buffer_count < _SSS_RX_BUFFER_SIZE) {		
                    ...
                    //Serial.print("_SSS_OP_FINAL ");Serial.print(_rx_buffer_count,DEC);Serial.print(" "); Serial.println(_rx_buffer[_rx_write_index-1],HEX);
					_rx_buffer_count++;
                }else{
#ifdef SERIALDEBUG			
				Serial.print("SlowSoftSerial Buffer full");
#endif				
			}
            }else{
#ifdef SERIALDEBUG						
				Serial.print("_SSS_OP_FINAL invalid discarding");
#endif								
			}
            // stop the timer and go back to waiting for a start bit.
            _rx_timer.enable(false);
